    ...
    return (
        ...
        <Modal fullscreen={true} show={ModalShow} onHide={() => setModalShow(false)}>
            ...
            <Modal.Body>
                ...
                {items[ModalItem].type === "web" ? <p><strong>GIT: </strong> {items[ModalItem].git.length > 0 ? <a href={items[ModalItem].git} target="_blank">{items[ModalItem].git}</a>:"Nezadáno, aktivní projekt"}</p> : null}
