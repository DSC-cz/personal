    ...
    return (
        ...
        <Modal fullscreen={true} show={ModalShow} onHide={() => setModalShow(false)}>
            ...
            <Modal.Body>
                ...
                {items[ModalItem].type === "web" || items[ModalItem].type === "others" ? <p><strong>GIT: </strong> {items[ModalItem].git.length > 0 ? <a href={items[ModalItem].git} target="_blank" rel="noopener noreferrer">{items[ModalItem].git}</a>:"Nezadáno, aktivní projekt"}</p> : <></>}
